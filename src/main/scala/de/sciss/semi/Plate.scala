...
object Plate {
  ...
  def apply(id: Int, transit: Boolean)(implicit tx: ProcTxn): Plate = {
    ...
    pColl1 ~> pAna
    pColl2 ~> SemiNuages.collMaster.audioInput(s"in${id + 1}")
    pColl1 ~> pRec
    pAna.play
    pDummy.dispose
    plate
...
class Plate(val id: Int, val collector1: Proc, val collector2: Proc, val analyzer: Proc, val recorder: Proc) {
  ...
  private val (injectIndex, injected) = {
    ...
    val names = if (names0.size <= MAX_FILES) names0
    else {
  ...
  def newAnalysis(loud0: Double, centr0: Double, flat0: Double)(implicit tx: ProcTxn) {
    ...
    if (eBal < -100) {
      ...
      if (prod) {
        val amt = rrand(EXHAUST_COUNT_LO, EXHAUST_COUNT_HI)
        exhaustedRef.transform(num => math.min(num + amt, MAX_EXHAUST_COUNT))
      }
    }
  }
  ...
  private def stopTooOldOne()(implicit tx: ProcTxn) {
    ...
    val fdt       = exprand(rp.context.minFade, rp.context.maxFade)
    val coinShade = coin(SHADE_PROB)
    if (coinShade) {
      ...
      ProcHelper.stopAndDispose(fdt, rp.proc)
      running.transform(_ - rp)
    }

    // XXX TODO could incorporate shade speed change info
    Analysis.log(s"$name-stop ${rp.context.name} fade ${(fdt * 44100L).toLong} shade $coinShade")
  }
  ...
  private def releaseSomeNoise()(implicit tx: ProcTxn): Boolean = {
    ...
    Analysis.log(s"$name-release ${rp.context.name} fade ${(fdt * 44100L).toLong}")
  ...
  private def produceSomeNoise()(implicit tx: ProcTxn): Boolean = {
    ...
    val durFr = nextPowerOfTwo((exprand(MIN_RECORD_DUR, MAX_RECORD_DUR) * SAMPLE_RATE).toInt)
    val dur   = durFr / SAMPLE_RATE
    recorder.control("dur").v_=(dur)
    //      collector ~> recorder
    recorder.play
    //      recordProc.set( Some( pRec ))

    Analysis.log(s"$name-produce dur $durFr")
  ...
  def recordDone(recPathF: File, ampInteg: Double)(implicit tx: ProcTxn) {
    ...
    val neighbour = choose((neighbour1 :: neighbour2 :: Nil).flatten)
    ...
    val docID = rand(4)
    val docTrns = (docID: @switch) match {
      case 0 =>
        Kriechstrom(
          in      = tmpA,
          out     = tmpB,
    ...
    Analysis.log(s"$name-rec-done neighbour ${neighbour.id} transform $docID")
  ...
  private def createProc(candidates: Seq[SoundContext])(implicit tx: ProcTxn): RunningProc = {
    val now = System.currentTimeMillis()
    val weightSum = candidates.foldLeft(0.0) {
      (sum, c) => sum + c.weight
    }
    val c = wchoose(candidates) {
      _.weight / weightSum
    }
    val f         = c.settings.createProcFactory(c.name)
    val deathDur  = exprand(c.minDur, c.maxDur)
    val death     = (deathDur * 1000).toLong + now
    val proc      = f.make
    ...
    val fdt       = exprand(rp.context.minFade, rp.context.maxFade)
    insertAndPlayGen(rp, fdt)

    Analysis.log(s"$name-create ${c.name} death-dur ${(deathDur * 44100L).toLong} fade ${(fdt * 44100L).toLong}")
